...
import ch.epfl.cs107.play.game.icrogue.actor.projectiles.poisonBalls;
...
public class PoisonTurret extends Enemy{
    ...
    public PoisonTurret(Area owner, Orientation orientation, DiscreteCoordinates coordinates, boolean sUp
    ...
    public void shootBall(){
        if (shootUp){
            poisonBalls ball = new poisonBalls(getOwnerArea(), Orientation.UP,
                new DiscreteCoordinates(getCurrentMainCellCoordinates().x
                ,(getCurrentMainCellCoordinates().y)+1), true, 1);

            ball.enterArea(getOwnerArea(),new DiscreteCoordinates(getCurrentMainCellCoordinates().x
                    , (getCurrentMainCellCoordinates().y)+1));
        }
        if (shootDown){
            poisonBalls ball = new poisonBalls(getOwnerArea(), Orientation.DOWN,
                    new DiscreteCoordinates(getCurrentMainCellCoordinates().x
                    ,(getCurrentMainCellCoordinates().y)-1), true, 1);
        ...
        if (shootLeft){
            poisonBalls ball = new poisonBalls(getOwnerArea(), Orientation.LEFT,
                    new DiscreteCoordinates((getCurrentMainCellCoordinates().x-1)
                    ,(getCurrentMainCellCoordinates().y)), true,1);

            ball.enterArea(getOwnerArea(),new DiscreteCoordinates((getCurrentMainCellCoordinates().x-1)
                    ,getCurrentMainCellCoordinates().y));
        }
        if (shootRight){
            poisonBalls ball = new poisonBalls(getOwnerArea(), Orientation.RIGHT,
                    new DiscreteCoordinates((getCurrentMainCellCoordinates().x+1)
                    ,(getCurrentMainCellCoordinates().y)), true,1);
