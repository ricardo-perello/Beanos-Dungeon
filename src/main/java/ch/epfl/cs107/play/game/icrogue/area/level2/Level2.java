...
public class Level2 extends Level {
    ...
    public void generateRandomMap(){
        ...
        Level2Room Room=new Level2Room(roomsToCreate.get(randomcoor));
        ...
        for(int i=0;i<roomArrangement[5];++i){
            ...
            xcor=roomsToCreate.get(randomcoor).x;
            ycor=roomsToCreate.get(randomcoor).y;
            Room=new Level2Room(coordinates);
            rooms.add(Room);
            setRoom(coordinates,Room);
            mapStates[xcor][ycor]=MapState.CREATED;
            roomsToCreate.remove(randomcoor);
    ...
    protected void setUpConnector(MapState[][] roomsplacement,ICRogueRoom room){
        ...
        if(xcor<roomsplacement.length-1&&(roomsplacement[xcor+1][ycor].equals(MapState.CREATED)||
            ...
            setRoomConnector(roomCoordinates,destinationName, Level2Room.Level2Connectors.E);
            boolean bossDestination=isNextToBossRoom(destination);
            if(bossDestination){
                lockRoomConnector(roomCoordinates, Level2Room.Level2Connectors.E,getBossRoomKeyID());
            }
        }
        if(xcor>0&&(roomsplacement[xcor-1][ycor].equals(MapState.CREATED)||
            ...
            String destinationName= getRoomName(destination);
            setRoomConnector(roomCoordinates,destinationName, Level2Room.Level2Connectors.W);
            boolean bossDestination=isNextToBossRoom(destination);
            if(bossDestination){
                lockRoomConnector(roomCoordinates, Level2Room.Level2Connectors.W,getBossRoomKeyID());
            }
        }
        if(ycor<roomsplacement[0].length-1&&(roomsplacement[xcor][ycor+1].equals(MapState.CREATED)||
            ...
            String destinationName= getRoomName(destination);
            setRoomConnector(roomCoordinates,destinationName, Level2Room.Level2Connectors.N);
            boolean bossDestination=isNextToBossRoom(destination);
            if(bossDestination){
                lockRoomConnector(roomCoordinates, Level2Room.Level2Connectors.N,getBossRoomKeyID());
            }
        }
        if(ycor>0 &&(roomsplacement[xcor][ycor-1].equals(MapState.CREATED)||
            ...
            boolean bossDestination=isNextToBossRoom(destination);
            String destinationName= getRoomName(destination);
            setRoomConnector(roomCoordinates,destinationName, Level2Room.Level2Connectors.S);
            if(bossDestination){
                lockRoomConnector(roomCoordinates, Level2Room.Level2Connectors.S,getBossRoomKeyID());
            }
        }
